...
def prior_behavior_funnel(events: pd.DataFrame) -> tuple[EvidenceTable, EvidenceTable]:
    """Audit strictly earlier view/cart events for each transaction event."""

    valid = _valid_events(events)
    valid["_source_order"] = np.arange(len(valid), dtype=np.int64)
    valid = valid.sort_values(
        ["visitorid", "itemid", "timestamp", "_source_order"], kind="mergesort"
    )

    transaction_rows: list[dict[str, int | bool]] = []
    intervals: list[int] = []
    for (_, _), group in valid.groupby(["visitorid", "itemid"], sort=False):
        prior_views: list[int] = []
        prior_carts: list[int] = []
        current_timestamp: int | None = None
        same_time_events: list[str] = []

        def flush(timestamp: int, rows: list[str]) -> None:
            has_view = bool(prior_views)
            has_cart = bool(prior_carts)
            last_view = prior_views[-1] if prior_views else None
            for event_value in rows:
                if event_value == "transaction":
                    transaction_rows.append(
                        {
                            "has_strictly_prior_view": has_view,
                            "has_strictly_prior_addtocart": has_cart,
                        }
                    )
                    if last_view is not None:
                        intervals.append(timestamp - last_view)
            if "view" in rows:
                prior_views.append(timestamp)
            if "addtocart" in rows:
                prior_carts.append(timestamp)

        for row in group.itertuples(index=False):
            timestamp = int(row.timestamp)
            event_value = str(row.event)
            if current_timestamp is None:
                current_timestamp = timestamp
            if timestamp != current_timestamp:
                flush(current_timestamp, same_time_events)
                current_timestamp = timestamp
                same_time_events = []
            same_time_events.append(event_value)
        if current_timestamp is not None:
            flush(current_timestamp, same_time_events)

    transaction_frame = pd.DataFrame(
        transaction_rows,
        columns=["has_strictly_prior_view", "has_strictly_prior_addtocart"],
    )
            ...
            [
                {
                    ...
                    "with_prior_view_count": 0,
                    "with_prior_addtocart_count": 0,
                    "with_prior_view_share": 0.0,
                    "with_prior_addtocart_share": 0.0,
                }
            ]
        )
    else:
        total = len(transaction_frame)
        prior_view_count = int(transaction_frame["has_strictly_prior_view"].sum())
        prior_cart_count = int(
            transaction_frame["has_strictly_prior_addtocart"].sum()
        )
        summary = pd.DataFrame(
            [
                {
                    "transaction_event_count": total,
                    "with_prior_view_count": prior_view_count,
                    "with_prior_addtocart_count": prior_cart_count,
                    "with_prior_view_share": prior_view_count / total,
                    "with_prior_addtocart_share": prior_cart_count / total,
                }
            ]
        )

    interval_series = pd.Series(intervals, dtype="Int64")
    interval_frame = distribution_summary(
        interval_series, "prior_view_to_transaction_interval_ms"
    )
    return (
        EvidenceTable(
        ...
        EvidenceTable(
            evidence_id="B05_VIEW_TO_TRANSACTION_INTERVAL",
            title="Most recent strictly prior view-to-transaction interval",
            population="transaction events with a strictly prior view for same user-item",
            denominator="transaction events with a strictly prior view for same user-item",
